...
paq_cleaning <- function(dataset) {
  ...
  paq_clean <- dataset %>%
    ...
    mutate(across(c("paq_3", "paq_6", "paq_9", "paq_12", "paq_15"),
                  ~ .x*24*60),
           # time sedentary reported in hours
           paq_16 = paq_16*60)
  ...
  paq_use <- paq_clean %>%
    ...
    filter(!if_all(grep("paq_[0-9]+", colnames(dataset), value=TRUE),
                   ~ is.na(.x))) %>%
    # max 16 hours (960 minutes)/day per activity (NA ok)
    filter(if_all(c(paq_3, paq_6, paq_9, paq_12, paq_15),
                  ~ (.x <= 960 | is.na(.x)))) %>%
    # max 7 days/week per activity (NA ok)
    filter(if_all(c(paq_2, paq_5, paq_8, paq_11, paq_14),
                  ~ (.x <= 7 | is.na(.x)))) %>%
    # max 24 hours (1440 minutes)/day sedentary (NA ok)
    filter(paq_16 <= 1440 | is.na(paq_16)) %>%
    rowwise() %>%
    ...
    mutate(count_correct = sum(
      c(
        ...
        # removed fully NA rows earlier with line 37
  ...
  paq_total <- paq_recs %>%
    ...
    scale_sum(oldvars = grep("WK", colnames(paq_recs), value=TRUE),
              ...
              newlab = "Minutes spent on physical activity per week",
  ...
  update_codebook(oldvars = "paq_total_week", 
                  ...
                  newlab = "Minutes spent on physical activity per day",
  ...
  paq_setting <- paq_total %>%
    scale_sum(oldvars = c("paq_12_WK", "paq_15_WK"),
              newvar = "paq_rec_day",
              newlab = "Minutes spent on recreation-related physical activities per day",
    ...
    scale_sum(oldvars = c("paq_9_WK"),
              ...
              newlab = "Minutes spent on travel-related physical activities per day",
              newchoices = "paq_9_WK divided by 7",
              position = "paq_total_day") %>%
    scale_sum(oldvars = c("paq_3_WK", "paq_6_WK"),
              newvar = "paq_work_day",
              newlab = "Minutes spent on work-related physical activities per day",

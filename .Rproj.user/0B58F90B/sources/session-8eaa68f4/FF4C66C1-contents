...
cxwalk <- read_rds(path_cxwalk)
id_check <- read_rds(path_id_check)
...
checkbox_key <- meta_raw %>%
  filter(field_type == "checkbox", !grepl("demo_placeholder", field_name)) %>%
  # clarify race demographic variables
  mutate(select_choices_or_calculations = case_when(
    field_name == "race" ~ "1, White | 2, Black or African American | 3, Asian | 4, Native Hawaiian or Other Pacific Islander | 5, American Indian or Alaska Native | 99, Unknown/Refuse to answer",
...
completes <- meta_raw %>%
  ...
  mutate(field_name = paste0(form_name, "_complete"),
         ...
         field_type = "radio",
         select_choices_or_calculations = "Complete | Incomplete | No Response")
...
meta_clean <- meta_raw %>%
  ...
  mutate(export_field_name = case_match(
    export_field_name,
    ...
    "race___7" ~ "race___4",
    "race___96" ~ "race___3",
    "record_id_80f34c" ~ "record_id",
    "grafana_id" ~ "mars_id",
    .default = export_field_name
  )) %>%
  mutate(export_field_name = str_replace_all(export_field_name, "^asi_", "aci_")) %>%
  mutate(
    # remove html syntax from labels
    field_label = str_remove_all(field_label, "<.*?>"),
    # clarify demographic coding
    choice_value = case_when(
      export_field_name == "race___5" ~ "5",
      export_field_name == "race___4" ~ "4",
      export_field_name == "race___3" ~ "3",
      .default = choice_value
      )) %>%
  # add response options for checkbox
  ...
  mutate(select_choices_or_calculations =
           case_when(
             ...
             # clarify demographic coding
             field_name == "ethnicity" ~ "0 - No | 1 - Yes | 99 - Unknown/Decline to Answer",
             .default = select_choices_or_calculations
           )) %>%
  ...
  rows_upsert(completes, by = "field_name") %>%
  rows_upsert(timestamps, by = "field_name") %>%
  # one row per response option
  separate_longer_delim(select_choices_or_calculations, 
                        delim = " | ") %>%
  mutate(select_choices_or_calculations = str_replace(select_choices_or_calculations, ", ", " - ")) %>%
  # remove superfluous variables (reflected in data)
  filter(!grepl("dses5_1|demo_placeholder", field_name) & !field_name %in% c("dses5a_1___nask", "subject_id")) %>%
  mutate(branching_logic = case_when(
    grepl("dses5a_1", field_name) ~ NA,
    .default = branching_logic
  ))
...
data_clean <- cxwalk %>%
  ...
  # combine V1 race questions
  mutate(dses5a_1___1 = case_when(dses4_1 == 1 ~ dses5_1___1,
                                 dses4_1 == 0 ~ dses5a_1___1),
         dses5a_1___2 = case_when(dses4_1 == 1 ~ dses5_1___2,
                                 dses4_1 == 0 ~ dses5a_1___2),
         dses5a_1___3 = case_when(dses4_1 == 1 ~ dses5_1___3,
                                 dses4_1 == 0 ~ dses5a_1___3),
         dses5a_1___4 = case_when(dses4_1 == 1 ~ dses5_1___4,
                                 dses4_1 == 0 ~ dses5a_1___4),
         dses5a_1___5 = case_when(dses4_1 == 1 ~ dses5_1___5,
                                 dses4_1 == 0 ~ dses5a_1___5)) %>%
  select(-c(starts_with("dses5_1"), dses5a_1___nask)) %>%
...
# check if demo_placeholder columns can be dropped
test <- data_clean %>%
  mutate(demo_placeholder_gender = case_match(demo_placeholder_gender,
                                              206 ~ "F",
                                              205 ~ "M")) %>%
  mutate(demo_placeholder_sex = case_match(demo_placeholder_sex,
                                           210 ~ 2,
                                           209 ~ 1)) %>%
  mutate(demo_placeholder_ethnicity = case_match(demo_placeholder_ethnicity,
                                                 203 ~ 970,
                                                 201 ~ 70)) %>%
  # demo variables had more NA values than the gender, scr_sex, etc., columns
  filter(!is.na(demo_placeholder_gender)) %>%
  select(record_id, starts_with("demo_placeholder"), gender, scr_sex, ethnicity, starts_with("race"))

# if TRUE, all values match and we can drop demo_placeholder questions
if (sum(c(
  all(test$demo_placeholder_gender == test$gender, na.rm=TRUE),
  all(test$demo_placeholder_sex == test$scr_sex, na.rm=TRUE),
  all(test$demo_placeholder_ethnicity == test$ethnicity, na.rm=TRUE),
  all(test$demo_placeholder_race___217 == test$race___3, na.rm=TRUE),
  all(test$demo_placeholder_race___218 == test$race___96, na.rm=TRUE),
  all(test$demo_placeholder_race___219 == test$race___7, na.rm=TRUE),
  all(test$demo_placeholder_race___220 == test$race___2, na.rm=TRUE),
  all(test$demo_placeholder_race___221 == test$race___1, na.rm=TRUE),
  all(test$demo_placeholder_race___222 == test$race___99, na.rm=TRUE))
) < 9) {
  stop("Not all values match")
}

data_clean <- data_clean %>%
  select(-starts_with("demo_placeholder")) %>%
  # clarify demographic coding
  mutate(ethnicity = case_match(ethnicity,
                                70 ~ 1,
                                970 ~ 0)) %>%
  rename(race___5 = race___3,
         race___4 = race___7,
         race___3 = race___96)
...
### 24. AI ----
# subscales
  ## Social Concerns:    aci_1, aci_6, aci_9, aci_11, aci_13, and aci_17
  ## Cognitive Concerns: aci_2, aci_5, aci_10, aci_14, aci_16, and aci_18
  ## Physical Concerns:  aci_3, aci_4, aci_7, aci_8, aci_12, and aci_15
# [TO DO]: check that these regex still work
data_clean24 <- data_clean23 %>%
  # no reverse coding
  # scale overall
  scale_mean(oldvars = grep("aci_[0-9]+$", colnames(data_clean), value=TRUE),
             newvar = "ai_overall",
             newlab = "Higher scores indicate higher anxiety",
             newchoices = "Mean of aci_1-aci_18",
             prop_nonmiss = 0.7,
             position = "aci_18") %>%
  # scale physical concerns
  scale_mean(oldvars = grep("(aci_[3,4,7,8])|(aci_1[2,5])", colnames(data_clean), value=TRUE),
             newvar = "ai_physical",
             newlab = "Higher scores indicate higher anxiety over physical concerns",
             newchoices = "Mean of aci_3, aci_4, aci_7, aci_8, aci_12, and aci_15",
             prop_nonmiss = 0.7,
             position = "aci_18") %>%
  # scale cognitive concerns
  scale_mean(oldvars = grep("(aci_[2,5])|(aci_1[0,4,6,8])", colnames(data_clean), value=TRUE),
             newvar = "ai_cognitive",
             newlab = "Higher scores indicate higher anxiety over cognitive concerns",
             newchoices = "Mean of aci_2, aci_5, aci_10, aci_14, aci_16, and aci_18",
             prop_nonmiss = 0.7,
             position = "aci_18") %>%
  # scale social concerns
  scale_mean(oldvars = grep("(aci_[1,6,9]$)|(aci_1[1,3,7])", colnames(data_clean), value=TRUE),
             newvar = "ai_social",
             newlab = "Higher scores indicate higher anxiety over social concerns",
             newchoices = "Mean of aci_1, aci_6, aci_9, aci_11, aci_13, and aci_17",
             prop_nonmiss = 0.7,
             position = "aci_18")
...
data_final <- data_clean49 %>%
  ...
  add_skip(skip_vars) %>%
  # ARDB v4: binge drinking questions weren't asked; manually fill in with -99
  mutate(binge_drink_male = if_else(redcap_event_name == "V4 Quest (6 month)" & dses1a_1 == 1, -99, binge_drink_male)) %>%
  mutate(binge_drink_female = if_else(redcap_event_name == "V4 Quest (6 month)" & dses1a_1 == 2, -99, binge_drink_female)) %>%
...
# get list of all items that patients were asked in each survey
var_groups <- meta_clean %>%
  filter(field_type != "summary", 
         !grepl("_complete|_timestamp", field_name),
         !field_name %in% identifiers,
         # keep screening survey and nudge tracking as is
         !form_name %in% c("basic_demography", "nudge_tracking_v2")) %>%
  select(field_name, form_name) %>%
  distinct() %>%
  unstack(field_name ~ form_name)

# recode completion of each survey (not exact in REDCap)
# keep screening survey as is
for (i in names(var_groups)) {
  vars <- var_groups[[i]]
  data_final <- data_final %>%
    mutate(!!paste0(i, "_complete") := case_when(
      if_all(vars, ~ !is.na(.x)) ~ "Complete",
      if_any(vars, ~ !is.na(.x)) & !if_all(vars, ~ !is.na(.x)) ~ "Incomplete",
      if_all(vars, ~ is.na(.x)) ~ "No Response",
    ))
}
...
for (i in 1:nrow(events_final)) {
  ...
  var_list <- c("record_id", "mars_id", event_mapping[event_mapping$event_name == events_final[[i, 1]],][["field_name"]])
  
  dataset <- data_final %>%
    filter(redcap_event_name == events_final[[i, 1]]) %>%
    # add in missing subject and grafana ID's
    right_join(id_mapping, join_by(record_id == record_id))%>%
    select(all_of(var_list)) %>%
    # mark all surveys from missing subjects as no response
    mutate(across(ends_with("_complete"), as.character)) %>%
    mutate(across(ends_with("_complete"),
                  ~ replace_na(.x, "No Response"))) %>%
    mutate(across(ends_with("_complete"),
                  ~ factor(.x, levels = c("Complete", "Incomplete", "No Response"))))
  
  c_vars <- grep("_complete", colnames(dataset), value=TRUE)

  dataset <- dataset %>%
    # add variable with overall completion
    mutate(!!as.name(paste0(events_final[[i, 2]], "_complete")) := case_when(
      if_all(c_vars, ~ .x == "Complete") ~ "Complete",
      if_any(c_vars, ~ .x %in% c("Complete", "Incomplete")) & !if_all(c_vars, ~.x == "Complete") ~ "Incomplete",
      if_all(c_vars, ~ .x == "No Response") ~ "No Response"),
    .after = mars_id) %>%
    mutate(!!as.name(paste0(events_final[[i, 2]], "_complete")) := 
             factor(!!as.name(paste0(events_final[[i, 2]], "_complete")),
                       levels = c("Complete", "Incomplete", "No Response")))
  
  # QC check: no duplicate ID's
  if(length(unique(dataset$record_id)) != nrow(dataset) | 
     length(unique(dataset$mars_id)) != nrow(dataset)) {
    stop("Duplicate ID's exist")
  }
  
  # QC check: max column name length for SAS/Stata
  if(max(str_length(colnames(dataset))) > 32) {
    stop("column names are too long")
  }
  
  datasets_list[[i]] <- dataset
  write_csv(dataset, paste0(path_output_data, events_final[[i, 2]], ".csv"))
  write_dta(dataset, paste0(path_output_data, events_final[[i, 2]], ".dta"))
  write_rds(dataset, paste0(path_output_data, events_final[[i, 2]], ".rds"))

  ### add worksheet to codebook ----
  code_temp <- meta_final %>%
    filter(field_name %in% var_list) %>%
    add_row(field_name = paste0(events_final[[i, 2]], "_complete"),
            field_label = paste("Completion status of entire", events_final[[i, 2]], "survey"),
            field_class = "categorical",
            field_type = "radio",
            select_choices_or_calculations = c("Complete", "Incomplete", "No Response")) %>%
...
saveWorkbook(codebook, paste0(path_output_data, "codebook.xlsx"), overwrite=TRUE)
